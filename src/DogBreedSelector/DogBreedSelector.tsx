...
export default function DogBreedSelector() {
  ...
  return (
    <div className="w-full max-w-lg min-w-[33vw] min-h-[33vh] mx-auto mt-10 p-6 rounded-2xl bg-white/10 backdrop-blur-lg shadow-2xl border border-white/10 text-white space-y-6 flex flex-col">
      <h2 className="text-2xl font-bold text-white-800 text-center">Select a Dog Breed</h2>
      ...
      {(selectedBreed === null && search !== "") && (<div className="max-h-64 overflow-y-auto rounded-lg border border-white/10 bg-white/5">
        {filtered.length === 0 && (
        ...
        {filtered.map((breed) => (
            <button
                key={breed}
                onClick={() => { setSelectedBreed(breed); setSearch(breed); }}
